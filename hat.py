...
#AWK command - AWKward command that doesn't sit well when called directly.
AWK = " " + "awk '!x[$0]++'" + " "
...
#raw_input(HASHCAT_PATH)
HASHCAT_EXE = "C:/Program Files/hashcat-6.1.1/hashcat.exe"
HASHCAT__PATH = "C:/Program Files/hashcat-6.1.1"
HASHCAT_PATH = "C:/Program Files/hashcat-6.1.1"


#Set the wordlist directory to where you're wordlists are...
WORDLIST_DIRECTORY = "C:/wordlists/"
L00T_POT_DIR = os.path.join(HAT_PATH, 'l00t')
RULES_DIR = os.path.join(HAT_PATH, 'rules')
STATS_DIR = os.path.join(HAT_PATH, 'stats')
HASH_UPLOAD_DIR = os.path.join(HAT_PATH, 'hash_upload')
CEWL_UPLOAD_DIR = os.path.join(HAT_PATH, 'cewl_wordlists')
TOOLS_DIR = os.path.join(HAT_PATH, 'tools')
RSMANGLER_UPLOAD_DIR = os.path.join(HAT_PATH, 'rsmangler')
RSMANGLER_INPUT_DIR = os.path.join(RSMANGLER_UPLOAD_DIR, 'input')
RSMANGLER_OUTPUT_DIR = os.path.join(RSMANGLER_UPLOAD_DIR, 'output')
WIRELESS_UPLOAD_DIR = os.path.join(HAT_PATH, 'wireless_upload')
...
def wordlist_walk():
    for dirpath, dirnames, files in os.walk(WORDLIST_DIRECTORY):
        for wordlist_filename in files:
            ...
            hc_cmd = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), abs_wordlist, '-w', '3', '-O']
...
def rule_set_walk():
    ...
    for root, dirs, files in os.walk(RULES_DIR):
        for filename in fnmatch.filter(files, exten):
            ...
            hc_cmd = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '-w', '3', '-O', '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', abs_rule_set]
            subprocess.call(hc_cmd)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0' or HM_ANSWER == '1' or HM_ANSWER == '2':
...
def crack_menu_0():
    ...
    global HASH_PATH_AND_NAME
    WORDLIST_DIRECTORY = "C:/wordlists/" #Needed to reset the wordlist directory.
    if ALL_MENU:
        pot_function()
    else:
        hash_mode_menu()
    ...
    if CEWL_BOOLEAN and FILE_HASH_BOOLEAN or SINGLE_HASH_BOOLEAN and CEWL_BOOLEAN:
        ...
        SINGLE_WORDLIST = os.path.join(WORDLIST_DIRECTORY, 'merged_list/sp00ks_merged_file_uniq.txt')
    hc_cmd1 = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'OneRuleToRuleThemAll.rule'), '-w', '3', '-O']
    subprocess.call(hc_cmd1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
        hc_cmd2 = [HASHCAT_EXE, '-m', HASH_TYPE, '-a', '0', '--username', '--session', 'all', '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--show', '-o', os.path.join(L00T_POT_DIR, POT + '.format2'), '--outfile-format', '2', HASH_PATH_AND_NAME]
        ...
        hc_cmd3 = [HASHCAT_EXE, '-m', HASH_TYPE, '--show', '-o', os.path.join(L00T_POT_DIR, POT + '.format2'), '--outfile-format', '2', HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--session', 'all']
...
def crack_menu_2():
    ...
    if ALL_MENU:
        ...
        pot_function()
        os.system('cls')
        hc = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'OneRuleToRuleThemAll.rule'), '-w', '3', '-O']
        subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
        ...
        hc_cmd3 = [HASHCAT_EXE, '-m', HASH_TYPE, '--show', '-o', os.path.join(L00T_POT_DIR, POT + '.format2'), '--outfile-format', '2', HASH_ABS_PATH, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--session', 'all']
        subprocess.call(hc_cmd3)
        subprocess.call(AWK + os.path.join(L00T_POT_DIR, POT + '.format2') + " | sort > " + os.path.join(L00T_POT_DIR, POT) + '.sorted ', shell=True)
        subprocess.call('rm' + ' ' + os.path.join(L00T_POT_DIR, POT + '.format2'), shell=True)
    else:
...
def crack_menu_3():
    ...
    if CEWL_BOOLEAN and FILE_HASH_BOOLEAN or SINGLE_HASH_BOOLEAN and CEWL_BOOLEAN:
        ...
        SINGLE_WORDLIST = os.path.join(WORDLIST_DIRECTORY, '4GB+/weakpass_2p')
    hc_cmd1 = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O']
    subprocess.call(hc_cmd1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_5():
    ...
    if ALL_MENU:
        ...
        pot_function()
        os.system('cls')
    hc = [HASHCAT_EXE, '-a', '6', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '?a?a?a?a', '-w', '3', '-O', '--increment']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_6():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('cls')
    hc = [HASHCAT_EXE, '-a', '7', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '?a?a?a?a', SINGLE_WORDLIST, '-w', '3', '-O', '--increment']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_7():
    ...
    hc1 = [HASHCAT_EXE, '-a', '7', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '?d?d?d?d', SINGLE_WORDLIST, '-w', '3', '-O', '--increment']
    subprocess.call(hc1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0' or HM_ANSWER == '1' or HM_ANSWER == '2':
    ...
    hc2 = [HASHCAT_EXE, '-a', '6', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '?d?d?d?d', '-w', '3', '-O', '--increment']
    subprocess.call(hc2)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
        ...
        hc_cmd3 = [HASHCAT_EXE, '-m', HASH_TYPE, '--show', '-o', os.path.join(L00T_POT_DIR, POT + '.format3'), '--outfile-format', '2', HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--session', 'all']
        subprocess.call(hc_cmd3)
        subprocess.call(AWK + os.path.join(L00T_POT_DIR, POT + '.format2') + " | sort > " + os.path.join(L00T_POT_DIR, POT) + '.sorted ', shell=True)
        subprocess.call('rm' + ' ' + os.path.join(L00T_POT_DIR, POT + '.format2'), shell=True)
    else:
...
def crack_menu_9():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('cls')
    hc = [HASHCAT_EXE, "-a", "0", "-m", HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'd3ad0ne.rule'), '-w', '3', '-O', '-v']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_10():
    ...
    if WIRELESS_BOOLEAN:
        ...
        hc_cmd2 = [HASHCAT_EXE, '-m', HASH_TYPE, '-a', '0', '--username', '--session', 'all', '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--show', '-o', os.path.join(L00T_POT_DIR, POT + '.format2'), '--outfile-format', '2', HASH_ABS_PATH]
        subprocess.call('rm' + ' ' + os.path.join(L00T_POT_DIR, POT + '.format2'), shell=True)
        subprocess.call(hc_cmd2)
        subprocess.call(AWK + os.path.join(L00T_POT_DIR, POT + '.format2') + " | sort > " + os.path.join(L00T_POT_DIR, POT) + '.sorted ', shell=True)
    elif HM_ANSWER == '1' or HM_ANSWER == '2':
...
def crack_menu_11():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('cls')
    hc1 = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'leetspeak.rule'), '-w', '3', '-O']
    subprocess.call(hc1)
    hc2 = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'unix-ninja-leetspeak.rule'), '-w', '3', '-O']
    subprocess.call(hc2)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_13():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('cls')
    hc = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'dive.rule'), '-w', '3', '-O']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_14():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('cls')
    hc1 = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'hob064.rule'), '-w', '3', '-O']
    subprocess.call(hc1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0' or HM_ANSWER == '1' or HM_ANSWER == '2':
        ...
        os.system('cls')
    hc2 = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'd3adhob0.rule'), '-w', '3', '-O']
    subprocess.call(hc2)
    if HM_ANSWER == '0':
        hc_cmd2 = [HASHCAT_EXE, '-m', HASH_TYPE, '-a', '0', '--username', '--session', 'all', '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--show', '-o', os.path.join(L00T_POT_DIR, POT + '.format2'), '--outfile-format', '2', HASH_PATH_AND_NAME]
        subprocess.call(hc_cmd2)
...
#Passphrase testing Menu (Option b)
def passphrase_menu():
    WORDLIST_DIRECTORY = "C:\wordlists"
    global DEFAULT_CEWL_FILE_OUTPUT
    global HASH_ABS_PATH
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('cls')
    hc = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'passphrase-rule1.rule'), '-r', os.path.join(RULES_DIR, 'passphrase-rule2.rule'), '-w', '3', '-O']
    subprocess.call(hc)
    if HM_ANSWER == '0' or HM_ANSWER == '1' or HM_ANSWER == '2':
        ...
        os.system('cls')
    hc1 = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'dive.rule'), '-w', '3', '-O']
    subprocess.call(hc1)
    hc2 = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'OneRuleToRuleThemAll.rule'), '-w', '3', '-O']
    subprocess.call(hc2)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def any_menu():
    if ALL_MENU:
        ...
        pot_function()
    os.system('cls')
    hc1 = [HASHCAT_EXE, '-a', '3', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--increment', "?a?a?a?a?a?a?a?a?a?a?a?a", '-w', '3', '-O']
    subprocess.call(hc1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def random_rules():
    ...
    if WIRELESS_BOOLEAN:
        ...
        hc = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O', '-g', '100']
    elif SELECTION == 'e':
        hc = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O', '-g', '1000']
    elif SELECTION == 'f':
        hc = [HASHCAT_EXE, '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O', '-g', '10000']
    subprocess.call(hc)
    if HM_ANSWER == '0':
        hc_cmd2 = [HASHCAT_EXE, '-m', HASH_TYPE, '-a', '0', '--username', '--session', 'all', '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--show', '-o', os.path.join(L00T_POT_DIR, POT + '.format2'), '--outfile-format', '2', HASH_PATH_AND_NAME]
        subprocess.call(hc_cmd2)
...
def crack_menu():
    ...
    try:
        while 1:
            ...
            try:
                SELECTION = raw_input("\n Select an Option: ")
                if SELECTION == "16" or SELECTION == "17" or SELECTION == "18" or SELECTION == "g" or SELECTION =="h":
                    raw_input('Not currently available on Windows, use Linux Version from git')
                else:
...
def hash_from_file():
    ...
    print""
    print'\033[34m' + ' ' + 'Below are the files in the hash upload directory.. (Hit Enter to Refresh)' + '\033[0m'
    print""
    ...
    try:
        if os.path.isfile(HASH_INPUT):
            if USER_ONLY_BOOLEAN == False:# Option 1
...
'''
#Disabled due to incompatability with Windows
#Wireless Menu
...
def report_menu():
    ...
    prCyan(L00T_POT_DIR)
    print""
    print'\033[34m' + ' ' + 'Below are the stats files created from sucessfully compromised passwords... (Hit Enter to Refresh)' + '\033[0m'
    print''
...
def hash_menu_full(): #Menu 7                                                                                                                 
    ...
    prCyan(HASH_UPLOAD_DIR)
    print''
    print'\033[34m' + ' ' + 'Below are the hash files in the hash upload directory.. (Hit Enter to Refresh)' + '\033[0m'
    print''
    for root, dirs, files in os.walk(HASH_UPLOAD_DIR):
        files.sort()
        for f in files:
    ...
    try:
        while 1:
            ...
            try:
                ...
                if task == "3" or task == "4" or task == "5" or task == "6" or task =="7":
                    raw_input('Not currently available on Windows, use Linux Version from git')
                options[task]()
            except KeyError:
                os.system('cls')
